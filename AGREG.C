...
36.8, 36.9, 37.0, 37.1, 37.2, 37.3, 37.4, 37.4, 37.5, 37.5,  /*210*/
...
unsigned char pacop=0; /*ª®ã£*/
unsigned char pbcop=0; /*ª®ã£*/
unsigned char pccop=0;  /*ª®ã£*/
...
char flproc[5]={0xff,0xff,0xff,0xff,0xff};  /*ª ­ «  ff-¨§¬ ;0-­¥ ¨§¬*/
unsigned char light[5]={12,12,12,12,12};
char buffbln[4][940];
float fff;
int mouse;  /* 0 - no   1- yes*/
int mast;
...
void prmscr(int ch)  /*¢ë¢®¤¨â ­  íªp ­ ppp prp ¨ â¤ ¨­®£¤  */
 {
	...
	int x,y,a,m;
	 ...
	 sprintf(bufer1,"P   =%3d    T   =%3d",(int)(avptr->prmk[ch].p),avptr->prmk[ch].t+1);
	 sprintf(bufer2,"   Š€€‹ %d",ch+1);
	 dam();
	 ...
	 sprintf(bufer1,"PPP =%3d    PRP =%3d",avptr->prmk[ch].ppp,avptr->prmk[ch].prp);
	 sprintf(bufer2,"   %s",(chn[ch]==0)?"‚›Š‹ž—…":"‚Š‹ž—…");
...
void imnk(int flp)
 {
	int x,x1=8,m;
	char bufer[5][5][17]={{"     …„€Ÿ     ","    Ž–…‘‘     ","     Š€€‹      ","    ‘ŠŽŽ‘’œ    ","     B›XO„      "},
		{"    “‘Š ‚‘…    ","    “‘Š  1     ","    “‘Š  2     ","    “‘Š  3     ","    “‘Š  4     "},
		{"    ‘’Ž ‚‘…    ","    ‘’Ž  1     ","    ‘’Ž  2     ","    ‘’Ž  3     ","    ‘’Ž 4      "},
		{"     Š€€‹      ","     …—€’œ     ","     ‚›•Ž„      ","    €‘—…’      ","  ‚›Ž ’Ž—Šˆ   "}};
	 m=mast;
...
int getmsel(void)
 {
	int x;
	 if((mse.y<(21*dly-10))||(mse.y>(22*dly-4))) return(-1);
	 for(x=0;x<5;x++)
		if((mse.x>(x*16*8+1+x0))&&(mse.x<(x*16*8+1+112+x0))) return(x);
	 return(-1);
 }
		 ...
		 switch(a)
			{
			 ...
			 case 59 : dam();help();eam();break;
			 case MOUSEV: if(getmsel()==-1) break;
						 inblok(x*16+1,21,14);
						 x=getmsel();
						 inblok(x*16+1,21,14);
...
void pppvp(int nkl,int *zdy,int *zdx,float *kf,int *oldy)
 {
	avptr->adclen[nkl]=0;
	...
	if(kf[nkl]!=0) if(avptr->prmk[nkl].p>avptr->prmk[nkl].ppl+dly/kf[nkl])
	 avptr->prmk[nkl].p=avptr->prmk[nkl].ppl+dly/kf[nkl];
	oldy[nkl]=(avptr->prmk[nkl].p-avptr->prmk[nkl].prl)*kf[nkl];

 }


void prpscr(int nkl,int fl)  /*¢ë¢®¤¨â ­  íªp ­  ¨§¬¥à¥­¨¥*/
 {
	 static int flo[4];
	 int x,y,m;
	 x=0;y=0;
	 m=mast;
	 if(flo[nkl]==fl) return;
	 flo[nkl]=fl;
	 dam();
	 switch(nkl)
		{
		 case 0 : x=32;  y=dly*5;  break;
		 case 1 : x=344; y=dly*5;  break;
		 case 2 : x=32;  y=dly*15; break;
		 case 3 : x=344; y=dly*15; break;
		}
		 setviewport(x+x0+10*8,y,x+x0+35*8,y+dly-2,1);
		 clearviewport();
		 if(fl==0) {if(m) eam();return;}
		 setcolor(ATCOLOR);
		 outtext("! ‚ H ˆ Œ € H ˆ … !");
		 setviewport(0,0,639+x0,dly*25-3,1);
		 if(m) eam();
...
void vproc(int nkl)
 {
	...
	int zdy[4],zdx[4],oldfl[4],prpnc[4];
	long prpn[4];
	int oldprp,ctm,delt;
	unsigned char tm=0;
	struct time tim;
	 ...
	 for(nkl=0;nkl<4;nkl++)
		{
		 ...
		 if(avptr->prmk[nkl].ppp-avptr->prmk[nkl].prp>0) pppvp(nkl,zdy,zdx,kf,oldy);
		 avptr->gbuf[nkl][0]=(avptr->prmk[nkl].p-avptr->prmk[nkl].prl)*kf[nkl];
		}
	 while(1)  /*à¨á®¢ ­¨¥ £à ä¨ª®¢ */
		{
		 do
		 ...
		 getch1(0x11);
		 for(nkl=0;nkl<4;nkl++) if(prpnc[nkl]==100)
			{
			 if(avptr->adclen[nkl]>478) flproc[nkl]=32;
			 if(flproc[nkl]==0xff)
				{
				 avptr->adclen[nkl]++;
				 ...
				 if(avptr->prmk[nkl].p>avptr->prmk[nkl].ppl+dly/kf[nkl])
					{
					 ...
					 kf[nkl]=gvs/(avptr->prmk[nkl].ppl-avptr->prmk[nkl].prl);
					 crdr(nkl,kf[nkl]);
					 for(ctm=0;ctm<avptr->adclen[nkl];ctm++)  /*for(ctm=0;ctm<avptr->adclen[nkl]-1;ctm++)*/
						{
...
/*--------------------------------------------*/
				 ...
				 if((avptr->adclen[nkl]==TIMETW)&&(i==0)&&(nkl<4))     /*[nkl]>10*/
		 ...
		 for(nkl=0;nkl<4;nkl++)
			{
			 if(flproc[nkl]==0xff)
				if(prpnc[nkl]<10)
				 {
					prpscr(nkl,1);
					prpn[nkl]=prpn[nkl]+avptr->prmk[nkl].p;
					prpnc[nkl]++;
					avptr->prmk[nkl].prp=prpn[nkl]/prpnc[nkl];
				 }
			 if(prpnc[nkl]==10)
				{
				 prpscr(nkl,0);
				 if ((avptr->prmk[nkl].ppp-avptr->prmk[nkl].prp) < 4)
					{ avptr->prmk[nkl].ppp=255; avptr->prmk[nkl].prp=16; }
				 avptr->prmk[nkl].ppl=avptr->prmk[nkl].ppp;
				 avptr->prmk[nkl].prl=avptr->prmk[nkl].prp;
				 crdr(nkl,gvs/(avptr->prmk[nkl].ppp-avptr->prmk[nkl].prp));
				 if(avptr->prmk[nkl].ppp-avptr->prmk[nkl].prp<=0) flproc[nkl]=32;
				 if(avptr->prmk[nkl].ppp-avptr->prmk[nkl].prp>0) pppvp(nkl,zdy,zdx,kf,oldy);
				 avptr->gbuf[nkl][0]=(avptr->prmk[nkl].p-avptr->prmk[nkl].prl)*kf[nkl];
				 prpnc[nkl]=100;
      ...
      if(oldfl[cn]!=flproc[cn])
       {
        ...
        oldfl[cn]=flproc[cn];      /*án - ª ã­â¥à*/
			...
			if(kbhit()||mf)  {sproc(mf);mf=0;}
			if(flproc[0]+flproc[1]+flproc[2]+flproc[3]==128)
			 {nosound();dam();if(m) eam();return;}
			gettime(&tim);
			tm=tim.ti_sec;
		 }
		dam();if(m) eam();
...
void sproc(int mf)  /*¯®ª  p¨áãîâáï ¬ «¥­ìª¨¥ £p ä¨ª¨*/
{
 int c,a;
 static int x;
 x=avptr->vs;
...
void grmene(int c) /*p¨á ¨ ¯®¤¯¨á ¡®«è á¨áâ ª®®p¤*/
{
 ...
 if(dy>0)
        ...
        for(k=0;zy-k*dy>5*dly;k++)             /*¯®¤¯¨áê y ª®®à¤¨­ âë ¢¢¥àx*/
        {
				 ...
				 if(f) {setcolor(BLUE);outtextxy(x0+46,zy-dy*k-3,"-");}
         if(old<k*((int)dy)-dly)
                {
								 if(!f) {setcolor(BLUE);outtextxy(x0+46,zy-dy*k-3,"-");}
                 setcolor(GBCOLOR);
        ...
        for(k=0;zy+k*dy<18*dly;k++)             /*¯®¤¯¨áê y ª®®à¤¨­ âë ¢­¨§*/
				{
				 setcolor(GBCOLOR);
				 if(100+k*20<1000) {sprintf(bff,"%3d%% -",100+k*20);cn=8*5;}
				 else if(100+k*20>999) {sprintf(bff,"%3d%%-",100+k*20);cn=8*4;}
				 setcolor(BLUE);
...
int getch1(int flk) /*á«¥¤¨â §  â¥¬¯*/
{
 ...
 char tc;
			...
			if(gpi)
			 {
				...
				if(tgr0<3) {setcolor(LIGHTBLUE);sprintf(buferf," t<20");}
				if(tgr0>240) {setcolor(LIGHTRED);sprintf(buferf," t>39");}
				outtext(buferf);
				setviewport(0,0,639+x0,dly*25-3,1);
				if(m) eam();
			 }
	...
	if(flk==0)
	 {
		if(me) {if(m) eam();{if(mse.btsr) return(CCANC);else return(MOUSEV);}}
		dam();
		a=getch();
		if(a==0) a=getch();
		while(kbhit()) getch();
...
void adraw(int newx,int chn)  /*p¨áã¥â á¨¬¢®« áâp¥«ª¨ */
{
 ...
 if(oldx!=0) {setcolor(PTCOLOR);outtextxy(newx+xd,(int)(ly+avptr->gbuf[chn][newx]*LBK),"");}
 oldx=newx;
} /**/
	 ...
	 for(a=0;((a!=27)&&(f==0));a=getch2(8))
		{
		 switch(a)
            ...
            if(avptr->pixx[chn][avptr->activpix[avptr->ach]]>5)
						 {
							x1=x; x=avptr->pixx[chn][avptr->activpix[avptr->ach]];
							setcolor(FON);
            ...
            outtextxy(x+xd,ly+avptr->gbuf[chn][x]*LBK+9,bufer);
						...
						cho=chn;
						return;
			 case 75 :if(x>0) {x--;f=0;}break;
			 case 77 :if(x<avptr->adclen[chn]) {x++;f=0;}break;  /*480*/
			 case 115 :if(x>10) {x-=10;f=0;}break;
			 case 116 :if(x<avptr->adclen[chn]-10) {x+=10;f=0;}break;   /*480*/
			 case 0  :break;
			}
		 if((x!=0)&&(a!=0))
			{
			 ...
			 if(avptr->activpix[avptr->ach]==3)
				{
				 ...
				 clearviewport();setcolor(FRCOLOR);outtext(bufer1);
				 setviewport(0,0,639+x0,dly*25-3,1);
				}
			}
		}
			 ...
			 switch(c)
				{
				 ...
				 case MOUSEV:
					if(mse.bts) {if(avptr->ach<3) avptr->ach++;break;}
					else if(mse.btsm) {if(avptr->ach>0) avptr->ach--;break;}
					else if(mse.btsr); else break;
...
void inipixx(void)
 {
	int n;
	char bufer[15];
	 setviewport(517+x0,14*dly,517+8*15+x0,19*dly+1,1);
	 clearviewport();
	 setviewport(0,0,639+x0,dly*25-3,1);
	 setcolor(RMCOLOR);
	 line(517+x0,dly*14,517+15*8+x0,dly*14);
	 line(517+15*8+x0,dly*14,517+15*8+x0,dly*19+1);
	 line(517+x0,dly*19+1,517+15*8+x0,dly*19+1);
	 line(517+x0,dly*14,517+x0,dly*19+1);
	 setcolor(OMCOLOR);
	 for(n=0;n<4;n++)  /*374*/
		{
		 setviewport(533+x0+2*8,(15+n)*dly,533+8*12+x0,(16+n)*dly,1);
		 sprintf(bufer,"%s %c","’O—ŠA",n+65);
		 outtext(bufer);
		}
	 setviewport(0,0,639+x0,dly*25-3,1);
	 inblok(65,15,14);
...
void pixcs(void) /*¢ë¡®p  ªâ â®çª¨*/
{
 ...
 m=mast;
 dam();
 buff=malloc(imagesize(517+x0,14*dly,517+8*15+x0,dly*19+1));
 getimage(517+x0,14*dly,517+8*15+x0,19*dly+1,buff);
 inipixx();
 eam();
 for(c=0;fl&&(c!=27);c=getch2(8))
 {
  switch(c)
  {
	 case 72 :inblok(65,15+y,14);if(y>0) y--;else y=3;inblok(65,15+y,14);break;
	 case 80 :inblok(65,15+y,14);if(y<3) y++;else y=0;inblok(65,15+y,14);break;
	 case MOUSEV:if(mse.btsr)
								{
                 dam();
								 setviewport(0,0,639+x0,dly*25-3,1);
								 putimage(517+x0,14*dly,buff,0);
								 free(buff);
								 if(m) eam();
								 return;
								}
		if(getprsel(65)==-1) break;
		if(getprsel(65)==0) break;
		 inblok(65,15+y,14);
		y=getprsel(65)-1;
		inblok(65,15+y,14);
		if(mse.ev);else break;
	 case 13:avptr->activpix[avptr->ach]=y;
					 dam();
					 setviewport(0,0,639+x0,dly*25-3,1);
					 avptr->fun=1;
					 buff1=malloc(imagesize(517+x0,14*dly,517+8*15+x0,dly*19+1));
					 getimage(517+x0,14*dly,517+8*15+x0,19*dly+1,buff1);
					 putimage(517+x0,14*dly,buff,0);
					 gbred(avptr->ach);
					 putimage(517+x0,14*dly,buff1,0);
					 free(buff1);
					 setviewport(10+x0,dly*23,600+x0+24,dly*24-1,1);
					 clearviewport();
					 setviewport(0,0,639+x0,dly*25-3,1);
...
sprintf(text,"    Ž–…Ž—Ž… ‚…ŒŸ %d ‘…Š  ",avptr->adclen[a]/20+1);
		 ...
		 switch(a)
			{
			 ...
			 case 64 : if(avptr->ach>3) break;
								 ...
								 if(x==3)  {dam();trner();eam();break;}
								 if(x==1)  {dam();calc(1);blank();eam();}
								 break;
			 case MOUSEV:if(getmsel()==-1) break;
						 inblok(x*16+1,21,14);
						 x=getmsel();
						 ...
						 if(mse.ev);
							else
							 {
								if(mse.btsr)
								 {
									if(x==3)  {dam();trner();eam();break;}
									if(x==1)  {dam();calc(1);blank();eam();break;}
								 }
								break;
							 }
			 case 13 : dam();
								 if(avptr->ach>3) break;
								 if(avptr->ach<0) break;
...
switch(x)
 ...
 case 1 : blnclr(); process(); break;
 case 2 : chnset();  break;
 case 3 : speedset();break;
 case 4 : c=-1;break;
 }
return c;
} /*mproc*/
	 ...
	 for(a=0;a!=68;a=getch1(0))
		{
		 switch(a)
			{
			 ...
			 case 75 : inblok(x*16+1,21,14);if(x>0) x--;else x=4;inblok(x*16+1,21,14);break;
			 case 77 : inblok(x*16+1,21,14);if(x<4) x++;else x=0;inblok(x*16+1,21,14);break;
			 case 60 : dam();h=-1;if(loadbvar()==27) return;eam();avptr->invflag=1; break;
			 case 61 : dam();h=-1;if(loadfnvar()==27)  break;eam();avptr->invflag=1; break;
			 case 59 : dam();h=-1;help();eam();break;
			 case 13 : dam();h=-1;
				 d=mproc(x);
				 if(d==1) {inblok(x*16+1,21,14);x++;inblok(x*16+1,21,14);}
				 if(d==-1) return;
				 eam();break;
			 case MOUSEV:
				if(getmsel()==-1) break;
		 ...
		 if(avptr->invflag==1) {inblok(x*16+1,21,14);avptr->invflag=0;for(a=0;a<4;a++) {avptr->prmk[a].t=0;avptr->fxl=1;dam();prmscr(a);eam();} }
		 if(a!=0) {if(h!=x) {mainh(x);h=x;}}
		 eam();
		}
 } /* mainm*/
...
main()
{
 ...
 setbkcolor(FON);
 mouse=0;
 if(mprm) if(mousecheck()) mouseinit();  /*ca¬¨ ¯o¬­ïâ o e£e, mouse ¨§¬e­ïeâ im*/
